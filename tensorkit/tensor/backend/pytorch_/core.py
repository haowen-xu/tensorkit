...
def abs(x: TensorLike) -> Tensor:
    return torch.abs(as_tensor(x))


def neg(x: TensorLike) -> Tensor:
    return torch.neg(as_tensor(x))


def exp(x: TensorLike) -> Tensor:
    return torch.exp(as_tensor(x))


def log(x: TensorLike) -> Tensor:
    return torch.log(as_tensor(x))


def log1p(x: TensorLike) -> Tensor:
    return torch.log1p(as_tensor(x))


def sin(x: TensorLike) -> Tensor:
    return torch.sin(as_tensor(x))


def cos(x: TensorLike) -> Tensor:
    return torch.cos(as_tensor(x))
...
def add(x: TensorLike, y: TensorLike) -> Tensor:
    return torch.add(as_tensor(x), as_tensor(y))


def sub(x: TensorLike, y: TensorLike) -> Tensor:
    return torch.sub(as_tensor(x), as_tensor(y))


def mul(x: TensorLike, y: TensorLike) -> Tensor:
    return torch.mul(as_tensor(x), as_tensor(y))


def fmod(x: TensorLike, y: TensorLike) -> Tensor:
    return torch.fmod(as_tensor(x), as_tensor(y))


def pow(x: TensorLike, y: TensorLike) -> Tensor:
    return torch.pow(as_tensor(x), as_tensor(y))
...
@jit
def _reduce_sum_sub(x, keepdims):
...
@jit
def _reduce_mean_sub(x, keepdims):
    # type: (Tensor, bool) -> Tensor
    if keepdims:
...
@jit
def _reduce_max_2(x, axis, keepdims):
    # type: (Tensor, List[int], bool) -> Tensor
    if len(axis) == 1:
        return torch.max(x, dim=axis[0], keepdim=keepdims)[0]
    else:
        for a in axis:
...
def reduce_max(x: TensorLike,
    ...
    if axis is None:
        ...
        return _reduce_max_2(x, axis, keepdims)


@jit
def _reduce_min_1(x, keepdims):
    # type: (Tensor, bool) -> Tensor
    if keepdims:
...
@jit
def _reduce_min_2(x, axis, keepdims):
    # type: (Tensor, List[int], bool) -> Tensor
    if len(axis) == 1:
        return torch.min(x, dim=axis[0], keepdim=keepdims)[0]
    else:
        for a in axis:
...
def reduce_min(x: TensorLike,
    ...
    if axis is None:
        ...
        return _reduce_min_2(x, axis, keepdims)
