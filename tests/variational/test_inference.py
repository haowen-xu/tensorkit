...
class VariationalInferenceTestCase(unittest.TestCase):
    ...
    def test_construction(self):
        vi = VariationalInference(Z.float_scalar(1.),
                                  Z.float_scalar(2.))
        ...
        np.testing.assert_equal(Z.to_numpy(vi.log_joint), 1.)
        np.testing.assert_equal(Z.to_numpy(vi.latent_log_joint), 2.)

    def test_errors(self):
        # test no sampling axis should cause errors
        vi = VariationalInference(
            Z.float_scalar(0.), Z.float_scalar(0.), axes=None)
        with pytest.raises(
    ...
    def test_elbo(self):
        log_p = Z.random.randn(shape=[5, 7])
        log_q = Z.random.randn(shape=[1, 3, 5, 7])

        # test without sampling axis
        vi = VariationalInference(log_p, log_q)
        output = vi.lower_bound.elbo()
        answer = elbo_objective(log_p, log_q)
        np.testing.assert_allclose(Z.to_numpy(output), Z.to_numpy(answer))
